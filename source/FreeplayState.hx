...
class FreeplayState extends MusicBeatState
{
	...
	[
		...
		FlxColor.fromRGB(161, 161, 161), // NMI
    ];
	...
	override function create()
	{
		...
		{
			Catagories = ['dave', 'joke', 'extras', 'dave2.5', 'classic', 'cover', 'fanmade', 'terminal', 'non-dave'];
			translatedCatagory = [
				...
				LanguageManager.getTextString('freeplay_non-dave')];
		}
		...
		switch (Catagories[CurrentPack].toLowerCase())
		{
			...
			case 'cover':
				...
				addWeek(['Foolhardy'], 2, ['zardyMyBeloved']);
         			addWeek(['Interdimensional-Kogre'], 4, ['dave-festival-3d']);
				if (FlxG.save.data.electricCockaldoodledooUnlocked)
			...
			case 'non-dave':
				addWeek(['nomoreinnocence'], 23, ['nmi']);
		}
	}
